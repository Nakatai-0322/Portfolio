<template>
	<b-container class="page  animate__animated animate__fadeIn">
		...
		<p>
			...
			on
			<em>
				<a href="https://dev.to/asaoluelijah" target="_blank" rel="noopener">dev.to</a>
			</em>
			<br />
			<br />
			<b-alert show variant="danger" v-if="showErrALert" style="display: inline-block;">
		...
		<b-row class="articles" v-if="blogs.length > 0">
			<div class="col-md-6" v-for="blog in blogs" :key="blog.id">
				<div>
					<b-card-group deck>
						<b-card :img-src="blog.image" :img-alt="blog.imgAlt" img-top tag="article" class="mb-2">
							<h4>{{ blog.title }}</h4>
							<b-badge class="animate__animated animate__fadeInUp" style="margin: 2px;"
								v-for="tag in blog.tags" :key="tag" pill :variant="
									tagVariants[Math.floor(Math.random() * tagVariants.length)]
								">#{{ tag }}</b-badge>
							<hr />
							<b-card-text>{{ blog.desc }}</b-card-text>

							<b-button :href="blog.url" target="_blank" rel="noopener" variant="default">Read
								More..</b-button>
						</b-card>
					</b-card-group>
				</div>
